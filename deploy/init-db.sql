...
-- followup_dispatch_queue (persistent follow-up processing)
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'followup_dispatch_queue' AND policyname = 'service_all') THEN
    CREATE POLICY service_all ON public.followup_dispatch_queue FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;
END $$;
CREATE INDEX IF NOT EXISTS idx_followup_dispatch_queue_workspace_status_date ON public.followup_dispatch_queue(workspace_id, status, dispatch_date, created_at);
CREATE INDEX IF NOT EXISTS idx_followup_dispatch_queue_instance_status ON public.followup_dispatch_queue(instance_name, status, dispatch_date);
