...
DECLARE _t text;
  _tables text[] := ARRAY[
    ...
    'group_message_queue','group_participant_events','group_queue_config','group_daily_stats',
...
DO $$ BEGIN
  IF to_regclass('public.api_request_logs') IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='api_request_logs' AND policyname='ws_select') THEN
