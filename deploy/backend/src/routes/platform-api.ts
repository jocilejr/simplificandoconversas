    ...
    if (amount !== undefined && amount !== null && reference_id) {
      ...
      if (!existingTx) {
        await sb.from("transactions").insert({
          ...
          amount: Number(amount),
          customer_phone: (await import("../lib/normalize-phone")).normalizePhone(cleaned),
          customer_name: customer_name || null,
          source: "api",
          type: type || "pix",
          status: "pendente",
    ...
    if (!mpResp.ok) {
      ...
      if (!existingRejected) {
        await sb.from("transactions").insert({
          ...
          customer_name, customer_phone: customer_phone ? (await import("../lib/normalize-phone")).normalizePhone(customer_phone) : null,
