...
import { Link2, Copy, RefreshCw, Users, MousePointerClick, CheckCircle2, XCircle, Trash2, Search, Plus } from "lucide-react";
...
export default function GroupSmartLinkTab() {
  ...
  const { smartLink, isLoading, stats, createSmartLink, updateSmartLink, deleteSmartLink, syncInviteLinks } = useGroupSmartLinks();
  ...
  const handleCopy = () => {
    if (!smartLink) return;
    const url = `${window.location.origin}/r/g/${smartLink.slug}`;
  ...
  // Determine active group (least members below limit)
  ...
  // ─── No smart link: creation form ───
  if (!smartLink && !isLoading) {
    return (
        ...
        <Card>
          <CardContent className="p-6 space-y-5">
            ...
            <p className="text-xs text-muted-foreground">
              Link inteligente que distribui leads automaticamente entre seus grupos WhatsApp.
            </p>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" onClick={handleFetchGroups} disabled={!instanceName || fetching}>
                  <Search className={`h-4 w-4 mr-1 ${fetching ? "animate-spin" : ""}`} />
                  Buscar Grupos
                </Button>
              </div>
            </div>
                    ...
                    <TableBody>
                      {fetchedGroups.map(g => (
                        <TableRow key={g.jid} className="cursor-pointer" onClick={() => toggleGroup(g.jid)}>
                          <TableCell>
                            <Checkbox checked={selectedJids.has(g.jid)} onCheckedChange={() => toggleGroup(g.jid)} />
                          </TableCell>
                          <TableCell className="text-sm truncate max-w-[250px]">{g.name}</TableCell>
                          <TableCell className="text-center">
                            <Badge variant="secondary" className="text-xs">{g.memberCount}</Badge>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                    ...
                    <TableRow key={gl.group_jid} className={isActive ? "bg-primary/5" : ""}>
                      ...
                      <TableCell className="text-center">
                        <Badge variant={isFull ? "destructive" : "secondary"} className="text-xs">
                          {gl.member_count || 0}/{maxMembersLimit}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell className="text-center">
                        {isActive ? (
                          <Badge className="text-xs bg-green-500/10 text-green-600 border-green-500/20">► Ativo</Badge>
                        ) : isFull ? (
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() => deleteSmartLink.mutate(smartLink.id)}
