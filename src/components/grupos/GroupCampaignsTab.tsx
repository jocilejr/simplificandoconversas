    ...
    <div className="space-y-4">
      ...
      {campaigns.length === 0 && !isLoading ? (
        ...
        <div className="grid gap-3">
          {campaigns.map((c: any) => (
            <Card key={c.id} className={`overflow-hidden border-border/50 transition-all ${c.is_active ? "border-primary/30" : ""}`}>
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-1 min-w-0 space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant="outline" className="gap-1 text-xs border-border/50">
                        {c.is_active ? <Zap className="h-3 w-3 text-primary" /> : <ZapOff className="h-3 w-3 text-muted-foreground" />}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <div className="flex gap-1 border-l border-border/50 pl-2 ml-1">
                      <Button size="sm" variant="outline" className="h-8 text-xs gap-1.5 px-2.5 border-border/50" onClick={() => setMessagesCampaign(c)}>
