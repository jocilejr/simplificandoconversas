...
import {
  ...
  FileText, ArrowLeft, Save, MailOpen, BarChart3,
} from "lucide-react";
...
export function EmailTemplatesTab() {
  const { templates, isLoading, templateStats, addTemplate, updateTemplate, deleteTemplate } = useEmailTemplates();
  ...
  const [sendDialogTemplate, setSendDialogTemplate] = useState<any | null>(null);
  ...
  return (
    <div className="space-y-4">
                ...
                {stats ? (
                  <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                    <span className="flex items-center gap-1">
                      <Send className="h-3 w-3" /> {stats.sent}
                    </span>
                    <span className="flex items-center gap-1">
                      <MailOpen className="h-3 w-3" /> {stats.opened}
                    </span>
                    <span className="flex items-center gap-1">
                      <BarChart3 className="h-3 w-3" /> {stats.openRate}%
                    </span>
                  </div>
                ) : (
                  <p className="text-[10px] text-muted-foreground/50">Sem envios</p>
                )}
                    ...
                    <TooltipProvider delayDuration={200}>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setSendDialogTemplate(t)}>
                            <Send className="h-3.5 w-3.5" />
                          </Button>
                        </TooltipTrigger>
                        <TooltipContent><p className="text-xs">Enviar</p></TooltipContent>
                      </Tooltip>
                      <Tooltip>
                        <TooltipTrigger asChild>
      ...
      <SendTemplateDialog
        template={sendDialogTemplate}
        onClose={() => setSendDialogTemplate(null)}
      />
    </div>
  );
}
...
function TemplateEditor({
  editingId, name, setName, subject, setSubject, htmlBody, setHtmlBody,
  previewMode, setPreviewMode, textareaRef, insertAtCursor,
  handleSave, handleSendPreview, sendingPreview, resetForm, isPending,
}: {
  editingId: string | null;
  name: string; setName: (v: string) => void;
  subject: string; setSubject: (v: string) => void;
  htmlBody: string; setHtmlBody: (v: string) => void;
  previewMode: "desktop" | "mobile"; setPreviewMode: (v: "desktop" | "mobile") => void;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={handleSave} disabled={isPending || !name.trim() || !subject.trim()}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome do template" className="bg-card border-border" />
        <Input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Assunto do e-mail (ex: Bem-vindo, {{nome}}!)" className="bg-card border-border" />
      </div>
