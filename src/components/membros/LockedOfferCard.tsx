...
interface Props { offer: Offer; themeColor: string; ownedProductNames?: string[]; ownedProductIds?: string[]; firstName?: string; memberProfile?: MemberProfile | null; memberPhone?: string; workspaceId?: string | null; customerName?: string; customerDocument?: string; }
type ChatBubble = { type: "text"; content: string } | { type: "image"; url: string };

const BUBBLE_DELAY_MS = 10000;

export default function LockedOfferCard({ offer, themeColor, ownedProductNames, ownedProductIds, firstName, memberProfile, memberPhone, workspaceId, customerName, customerDocument }: Props) {
  ...
  return (
    <>
      ...
      <PaymentFlow open={paymentOpen} onOpenChange={setPaymentOpen} offer={offer} themeColor={themeColor} memberPhone={memberPhone || ""} workspaceId={workspaceId} customerName={customerName} customerDocument={customerDocument} />
    </>
  );
}
